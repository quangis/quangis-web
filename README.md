...
implementation is in [this repository][qblo].
...
## Setup question parser as SOA server

The question parser has its own Github repository [here][gqp]. Follow the guide in the README file to install the question parser and run it as a service.

## Setup Django server

#### Configuring a conda environment for the Django server

1. Install the 64-Bit version of [Miniconda 4.10.3][cnd] (Windows,MaxOS and Linux). Skip this step if you have already done so in the previous guides.

2. Install [Git][git]. Skip this step if you have already done so in the previous guides.
...
```
17. Run ‘*runApache.bat*’ within the miniconda prompt.

18. To test if the server is running properly, try opening the server address (e.g., http://127.0.0.1/) in a browser.

19. To stop the server press the combo “*Ctrl+C*” in the anaconda prompt. Enter ‘*Y*’ when asked “*Terminate batch job (Y/N)?*”.
...
[qblo]: https://github.com/quangis/quangis-blockly
[gqp]: https://github.com/quangis/geo-question-parser/tree/haiqi
